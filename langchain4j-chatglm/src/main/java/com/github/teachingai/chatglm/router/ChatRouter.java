...
import dev.langchain4j.model.chatglm.ChatGlmStreamingChatModel;
...
public class ChatRouter {
    ...
    public static void register(Javalin app, AppConfig config) {
        // 创建聊天模型
        ChatLanguageModel chatModel = ChatGlmChatModel.builder()
                .apiKey(config.getChatGlmApiKey())
                .modelName(config.getChatModel())
                .temperature(config.getTemperature())
                .build();

        StreamingChatLanguageModel streamingChatModel = ChatGlmStreamingChatModel.builder()
                .apiKey(config.getChatGlmApiKey())
                .modelName(config.getChatModel())
                .temperature(config.getTemperature())
                .build();
