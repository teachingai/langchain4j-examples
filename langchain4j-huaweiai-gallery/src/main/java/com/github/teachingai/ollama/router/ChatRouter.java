...
public class ChatRouter {
    ...
    public static void register(Javalin app, AppConfig config) {
        // 创建聊天模型
        // 注意：这里需要根据实际的langchain4j-huaweiai-gallery库的API来调整
        ChatLanguageModel chatModel;
        StreamingChatLanguageModel streamingChatModel = null;
        
        try {
            // 尝试使用HuaweiAI Gallery特定的模型类
            Class<?> chatModelClass = Class.forName("dev.langchain4j.model.huaweiai.gallery.HuaweiAiGalleryChatModel");
            Object builder = chatModelClass.getMethod("builder").invoke(null);
            builder = builder.getClass().getMethod("apiKey", String.class).invoke(builder, config.getHuaweiAiGalleryApiKey());
            ...
            // 尝试创建流式模型
            try {
                Class<?> streamingModelClass = Class.forName("dev.langchain4j.model.huaweiai.gallery.HuaweiAiGalleryStreamingChatModel");
                Object streamingBuilder = streamingModelClass.getMethod("builder").invoke(null);
                streamingBuilder = streamingBuilder.getClass().getMethod("apiKey", String.class).invoke(streamingBuilder, config.getHuaweiAiGalleryApiKey());
            ...
            log.error("Failed to create HuaweiAI Gallery chat model. Please check if langchain4j-huaweiai-gallery dependency is correct.", e);
            throw new RuntimeException("Failed to initialize HuaweiAI Gallery chat model", e);
        }
