...
import com.github.hiwepy.huaweiai.pangu.config.AppConfig;
...
public class ChatRouter {
    ...
    public static void register(Javalin app, AppConfig config) {
        // 创建聊天模型
        // 注意：这里需要根据实际的langchain4j-huaweiai-pangu库的API来调整
        ChatLanguageModel chatModel;
        StreamingChatLanguageModel streamingChatModel = null;
        
        try {
            // 尝试使用HuaweiAI Pangu特定的模型类
            Class<?> chatModelClass = Class.forName("dev.langchain4j.model.huaweiai.pangu.HuaweiAiPanguChatModel");
            Object builder = chatModelClass.getMethod("builder").invoke(null);
            builder = builder.getClass().getMethod("apiKey", String.class).invoke(builder, config.getHuaweiAiPanguApiKey());
            builder = builder.getClass().getMethod("secretKey", String.class).invoke(builder, config.getHuaweiAiPanguSecretKey());
            ...
            // 尝试创建流式模型
            try {
                Class<?> streamingModelClass = Class.forName("dev.langchain4j.model.huaweiai.pangu.HuaweiAiPanguStreamingChatModel");
                Object streamingBuilder = streamingModelClass.getMethod("builder").invoke(null);
                streamingBuilder = streamingBuilder.getClass().getMethod("apiKey", String.class).invoke(streamingBuilder, config.getHuaweiAiPanguApiKey());
                streamingBuilder = streamingBuilder.getClass().getMethod("secretKey", String.class).invoke(streamingBuilder, config.getHuaweiAiPanguSecretKey());
            ...
            log.error("Failed to create HuaweiAI Pangu chat model. Please check if langchain4j-huaweiai-pangu dependency is correct.", e);
