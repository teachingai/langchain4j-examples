...
public class ChatRouter {
    ...
    public static void register(Javalin app, AppConfig config) {
        // 创建工具规范
        List<ToolSpecification> toolSpecifications = FunctionConfig.createToolSpecifications();
        ...
        app.get("/v1/prompt", new Handler() {
            ...
            public void handle(Context ctx) throws Exception {
                Map<String, Object> variables = Map.of(
                        "adjective", ctx.queryParam("adjective", "funny"),
                        "topic", ctx.queryParam("topic", "cats")
                );
                PromptTemplate promptTemplate = PromptTemplate.from("Tell me a {adjective} joke about {topic}");
                Prompt prompt = promptTemplate.apply(variables);
                Response<AiMessage> response = chatModel.generate(List.of(new UserMessage(prompt.text())), toolSpecifications);
                ctx.json(response);
            }
        });
