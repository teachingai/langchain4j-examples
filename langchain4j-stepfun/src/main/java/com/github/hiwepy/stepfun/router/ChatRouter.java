...
public class ChatRouter {
    ...
    public static void register(Javalin app, AppConfig config) {
        // 创建聊天模型
        // 注意：这里需要根据实际的langchain4j-stepfun库的API来调整
        ChatLanguageModel chatModel;
        StreamingChatLanguageModel streamingChatModel = null;
        
        try {
            // 尝试使用StepFun特定的模型类
            Class<?> chatModelClass = Class.forName("com.github.hiwepy.langchain4j.stepfun.StepFunChatModel");
            Object builder = chatModelClass.getMethod("builder").invoke(null);
            builder = builder.getClass().getMethod("apiKey", String.class).invoke(builder, config.getStepFunApiKey());
            ...
            // 尝试创建流式模型
            try {
                Class<?> streamingModelClass = Class.forName("com.github.hiwepy.langchain4j.stepfun.StepFunStreamingChatModel");
                Object streamingBuilder = streamingModelClass.getMethod("builder").invoke(null);
                streamingBuilder = streamingBuilder.getClass().getMethod("apiKey", String.class).invoke(streamingBuilder, config.getStepFunApiKey());
            ...
            log.error("Failed to create StepFun chat model. Please check if langchain4j-stepfun dependency is correct.", e);
            throw new RuntimeException("Failed to initialize StepFun chat model", e);
        }
